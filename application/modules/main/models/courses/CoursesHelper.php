...
class CoursesHelper extends APP_Model
{
    ...
    /**
     * получить информацию по курсу для оплаты с ленда
     * @param type $code
     * @return type
     */
    public function getCourseForExternalPay($code = null)
    {
        if (empty($code)) {
            return null;
        }
        
        $item = $this->CoursesModel->getByCode($code);
        if (empty($item)) {
            return null;
        }
        
//        $this->CoursesHelper->prepareItem($item);
        $offers = $this->GroupsModel->listOffersForCourse($item['id']);
        $item['offers'] = [];
        $item['next_offer'] = null;
        
        if (count($offers)) {
            $item['offers'] = $offers;
            $item['next_offer'] = current($offers);
        }
